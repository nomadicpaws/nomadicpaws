...
// ── Shared Nav (absolute paths so it works correctly from any subpage) ──
const NAV_HTML = `
<nav id="mainNav">
...
<style id="np-shared-nav-styles">
    ...
    #mainNav .nav-cta:hover { background: #E8845A; transform: translateY(-1px); }
...
// ── Shared Footer (absolute paths) ──
const FOOTER_HTML = `
<footer style="background:#2C1810;padding:64px 40px 32px;color:rgba(253,250,245,0.5);font-family:'DM Sans', sans-serif;">
    <div style="max-width:1100px;margin:0 auto;display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:48px;margin-bottom:48px;">
        <div>
        ...
        <div>
            ...
            <div style="display:flex;gap:16px;">
                <a href="https://tiktok.com/@nomadicpawstv" target="_blank" style="color:rgba(253,250,245,0.5);font-size:1.15rem;text-decoration:none;"><i class="fab fa-tiktok"></i></a>
                <a href="https://youtube.com/@nomadicpawstv" target="_blank" style="color:rgba(253,250,245,0.5);font-size:1.15rem;text-decoration:none;"><i class="fab fa-youtube"></i></a>
                <a href="https://instagram.com/nomadicpawstv" target="_blank" style="color:rgba(253,250,245,0.5);font-size:1.15rem;text-decoration:none;"><i class="fab fa-instagram"></i></a>
            </div>
        </div>
        <div>
...
export default async (request: Request, context: Context) => {
  ...
  return new HTMLRewriter()
    .on("#nav-slot", {
      element(el) {
        el.replace(NAV_STYLES + NAV_HTML, { html: true });
      },
    })
    .on("#footer-slot", {
      element(el) {
        el.replace(FOOTER_HTML + NAV_SCRIPT, { html: true });
      },
    })
    .transform(response);
};

// Add a new path string here any time you create a new standalone page —
// that's the only maintenance this setup ever needs.
export const config = {
  path: ["/checklist/*", "/cheetos-store/*"],
};
