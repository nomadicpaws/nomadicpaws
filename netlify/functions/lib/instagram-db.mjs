...
export async function getInstagramStudio() {
  const [settings, templates, posts] = await Promise.all([
    ...
    getDatabase().pool.query(`SELECT id, title, caption, media_urls, target_date, theme, status, created_at, updated_at FROM instagram_post_drafts WHERE owner_name = 'Trinitie' ORDER BY target_date NULLS LAST, updated_at DESC`),
  ])
...
export async function saveInstagramPost(input) {
  ...
  const result = await getDatabase().pool.query(
    `INSERT INTO instagram_post_drafts (id, owner_name, title, caption, media_urls, target_date, theme, status, created_at, updated_at)
     VALUES ($1, 'Trinitie', $2, $3, $4::jsonb, NULLIF($5::text, '')::date, $6, $7, NOW(), NOW())
     ON CONFLICT (id) DO UPDATE SET title = EXCLUDED.title, caption = EXCLUDED.caption, media_urls = EXCLUDED.media_urls,
       target_date = EXCLUDED.target_date, theme = EXCLUDED.theme, status = EXCLUDED.status, updated_at = NOW()
     WHERE instagram_post_drafts.owner_name = 'Trinitie'
     RETURNING id, title, caption, media_urls, target_date, theme, status, created_at, updated_at`,
    [id, input.title.trim(), input.caption, JSON.stringify(input.mediaUrls), input.targetDate || '', input.theme.trim(), input.status],
  )
  return result.rows[0]
}
