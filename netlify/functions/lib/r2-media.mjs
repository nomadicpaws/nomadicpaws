...
export function normalizeR2Endpoint(endpoint, bucket) {
  const cleanEndpoint = String(endpoint || '').replace(/\/+$/, '')
  const encodedBucket = encodeURIComponent(String(bucket || ''))
  if (!cleanEndpoint || !encodedBucket) return cleanEndpoint
  return cleanEndpoint.replace(new RegExp(`/${encodedBucket}$`, 'i'), '')
}

function settings() {
  const bucket = String(process.env.R2_BUCKET || '')
  return {
    endpoint: normalizeR2Endpoint(process.env.R2_ENDPOINT, bucket),
    bucket,
