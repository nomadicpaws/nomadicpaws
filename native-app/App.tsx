...
import { addReviewNote, API_URL, JournalReviewNote, JournalStory, JournalStoryDetail, loadStories, loadStory, signIn } from './src/api'
import { ContentSeed, initialSchedule, Person, PreviewReaction, SharedPreview, starterPreviews, starterSeeds } from './src/content'
...
function Journal({ token, person }: { token: string; person: Person }) {
  const [stories, setStories] = useState<JournalStory[]>([]), [selected, setSelected] = useState<JournalStoryDetail>(), [notes, setNotes] = useState<JournalReviewNote[]>([]), [reviewNote, setReviewNote] = useState(''), [loading, setLoading] = useState(true), [opening, setOpening] = useState(false), [saving, setSaving] = useState(false), [message, setMessage] = useState(''), [error, setError] = useState('')
  ...
  async function open(story: JournalStory) { setOpening(true); setError(''); try { const data = await loadStory(token, story.slug); setSelected(data.story); setNotes(data.notes) } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to open this story.') } finally { setOpening(false) } }
  async function submitReview() { if (!selected || person === 'Katie' || !reviewNote.trim()) return; setSaving(true); setError(''); setMessage(''); try { const data = await addReviewNote(token, { slug: selected.slug, version: selected.version, reviewer: person, note: reviewNote.trim() }); setNotes(current => [data.note, ...current]); setReviewNote(''); setMessage('Your review note is saved.') } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to save this note.') } finally { setSaving(false) } }
...
function Today({ person, onPersonChange, seeds, onNewAdventure, onOpenPreviews }: { person: Person; onPersonChange: (person: Person) => void; seeds: ContentSeed[]; onNewAdventure: () => void; onOpenPreviews: () => void }) {
